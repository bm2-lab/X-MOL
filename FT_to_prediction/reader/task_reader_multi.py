...
class BaseReader(object):
    ...
    def _truncate_seq_multi(self, tokens_parts, max_length):
        """Truncates a sequence group in place to the maximum length."""

        # This is a simple heuristic which will always truncate the longest sequence
        # one token at a time. This makes more sense than truncating an equal percent
        # of tokens from each, since if one sequence is very short then each token
        # that's truncated likely contains more information than a longer sequence.
        while True:
            length = [len(i) for i in tokens_parts]
            total_length = sum(length)
            if total_length <= max_length:
                break
            maxlen_loc = 0
            maxlen = 0
            for idx in range(len(tokens_parts)):
                if length[idx] > maxlen:
                    maxlen_loc = idx
            tokens_parts[idx].pop()
    ...
    def _convert_example_to_record(self, example, max_seq_length, tokenizer):
        ...
        input_texts = [i for i in example]
        label = input_texts[-1]
        input_seq = input_texts[:-1]
        type_num = len(input_seq)
        bear_max_len = max_seq_length - type_num - 1
        tokens = []
        tokens.append("[CLS]")
        text_type_ids = []
        text_type_ids.append(0)
        tokens_parts = []
        for type_id, input_part in enumerate(input_seq):
            tokens_part = tokenization.convert_to_unicode(input_part)
            tokens_part = tokenizer.tokenize(tokens_part)
            tokens_parts.append(tokens_part)

        self._truncate_seq_multi(tokens_parts, bear_max_len)

        for type_id, tokens_part in enumerate(tokens_parts):
            tokens += tokens_part
            for _ in tokens_part:
                text_type_ids.append(type_id)
            tokens.append("[SEP]")
            text_type_ids.append(type_id)
