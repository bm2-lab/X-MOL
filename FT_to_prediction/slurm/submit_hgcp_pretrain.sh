...
cp -r ./model/ ./reader/ ./utils/ ./package/ ./script/ ./slurm ./$job_package_path/

set +x
cd $job_package_path
ln -s model_conf ./$conf
ln -s ./slurm/train_pretrain.sh ./train.sh
ln -s ./slurm/job.sh ./job.sh
ln -s ./pretrain_launch.py ./lanch.py

if [[ ! -f $platform || $platform == "paddlecloud" ]];then
    ln -s ./slurm/submit_paddlecloud_pretrain.sh ./submit.sh
else
    ln -s ./slurm/submit_pretrain.sh ./submit.sh
fi
