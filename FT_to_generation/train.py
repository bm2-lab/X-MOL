...
def create_model(pyreader_name, ernie_config):
    input_mask_shape = [-1, args.max_seq_len, args.max_seq_len] \
        if args.is_unidirectional else [-1, args.max_seq_len, 1]
    pyreader = fluid.layers.py_reader(
        capacity=70,
        shapes=[[-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1], 
                [-1, args.max_seq_len, 1], input_mask_shape,
                [-1, 1], [-1, 1], [-1, 1]],
        dtypes=[
    ...
    ernie = ErnieModel(
        ...
        is_unidirectional=args.is_unidirectional,
        two_stream=False)
    
    next_sent_acc, mask_lm_loss_context, total_loss = ernie.get_pretraining_output(
        mask_label, mask_pos, labels)

    return pyreader, next_sent_acc, mask_lm_loss_context, total_loss
...
def predict_wrapper(args,
    ...
    def predict(exe=exe, pyreader=pyreader):
        ...
        while True:
            try:
                each_next_acc, each_mask_lm_context_cost, each_total_cost = exe.run(
                    fetch_list=fetch_list, program=test_prog)
                acc += each_next_acc
                lm_cost += each_mask_lm_context_cost
...
def train(args):
    ...
    with fluid.program_guard(train_program, startup_prog):
        with fluid.unique_name.guard():
            train_pyreader, next_sent_acc, mask_lm_loss_c, total_loss = create_model(
    ...
    with fluid.program_guard(test_prog, startup_prog):
        with fluid.unique_name.guard():
            test_pyreader, next_sent_acc, mask_lm_loss_c, total_loss = create_model(
                pyreader_name='test_reader', ernie_config=ernie_config)
    ...
    if args.valid_filelist and args.valid_filelist != "":
        predict = predict_wrapper(
            ...
            pyreader=test_pyreader,
            fetch_list=[
                next_sent_acc.name, mask_lm_loss_c.name, total_loss.name
            ])
    ...
    while True:
        try:
            ...
            if nccl2_trainer_id == 0 and steps % skip_steps == 0:
                fetch_list = [next_sent_acc.name, mask_lm_loss_c.name, 
            ...
            if ret:
                each_next_acc, each_mask_lm_context_cost, each_total_cost, np_lr, l_scaling = ret \
                    if args.use_fp16 else ret + [[args.init_loss_scaling]]
                
                acc.extend(each_next_acc)
                lm_cost.extend(each_mask_lm_context_cost)
                ...
                print(
                    ...
                    "ppl-c: %f, next_sent_acc: %f, speed: %f steps/s, file: %s, mask_type: %s"
            ...
            if args.valid_filelist and steps % args.validation_steps == 0:
                vali_cost, vali_lm_c_cost, vali_acc, vali_steps, vali_speed = predict(
                )
                print("[validation_set] epoch: %d, step: %d, "
                      "loss: %f, global ppl-c: %f, batch-averged ppl-c: %f, "
                      "next_sent_acc: %f, speed: %f steps/s" %
                      (epoch, steps, np.mean(np.array(vali_cost) / vali_steps),
                       np.exp(np.mean(np.array(vali_lm_c_cost) / vali_steps)),
                       np.mean(np.exp(np.array(vali_lm_c_cost) / vali_steps)),
