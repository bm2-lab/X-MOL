...
class Seq2SeqReader(object):
    ...
    def _read_tsv(self, input_file, quotechar=None):
        ...
        with open(input_file, "r") as f:
            ...
            Example = namedtuple('Example', ["src", "label", "tgt", "knowledge", "data_id"])
            ...
            for line in reader:
                ...
                for index, text in enumerate(line):
                    if index in src_indices:
                        src.append("")
                        label.append(float(text))
                    elif headers[index] == "tgt":
                        tgt = text
                    else:
                        knowledge = text

                examples.append(Example(src=src, label=label, tgt=tgt, knowledge=knowledge, data_id=data_id))
                data_id += 1
    ...
    def _convert_example_to_record(self, example, tokenizer, do_dec=False):
        ...
        Record = namedtuple(
            'Record',
            ['token_ids', 'text_type_ids', 'position_ids', 'tgt_start_idx', 'data_id', 'gen_tgt'])
        record = Record(
            token_ids=token_ids,
    ...
    def _pad_batch_records(self, batch_records, do_dec, place):
        ...
        batch_gen_tgt = [record.gen_tgt for record in batch_records]
        batch_gen_tgt = np.array(batch_gen_tgt)
        ...
        if self.is_dialogue_task:  
            ...
            return_list = [
                batch_gen_tgt, padded_token_ids, padded_text_type_ids, padded_position_ids,
                input_mask
            ]
