...
def multi_head_attention(queries,
    ...
    def scaled_dot_product_attention(q, k, v, attn_bias, d_key, dropout_rate, tag=''):
        """
        ...
        if tag != '':
            tag = tag + '@'+ str(round(time(),2))[4:]
            layers.Print(weights, summarize=-1, message=tag)
        if dropout_rate:
...
def two_stream_encoder(enc_input_context,
    ...
    for i in range(n_layer):
        if enc_input_context.dtype == fluid.core.VarDesc.VarType.FP16:
            k_v = layers.cast(layers.concat([layers.cast(enc_input_context,'float32'), layers.cast(enc_input_query,'float32')],axis=1),'float16')
        else:
            k_v = layers.concat([enc_input_context, enc_input_query],axis=1)

        enc_output_context = encoder_layer(
            enc_input_context,
            None,
            attn_bias_context,
            n_head,
    ...
    enc_output_context = pre_process_layer(
        enc_output_context, preprocess_cmd, prepostprocess_dropout, name="post_encoder", epsilon=epsilon)
    enc_output_query = pre_process_layer(
        enc_output_query, preprocess_cmd, prepostprocess_dropout, name="post_encoder", epsilon=epsilon)

    return enc_output_context, enc_output_query
