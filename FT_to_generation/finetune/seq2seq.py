...
def fast_decode(args, pyreader_name, ernie_config):
    if args.task_type == "dialog":
        ...
        pyreader = fluid.layers.py_reader(
            ...
            shapes=[[-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1],
                ...
                [-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1], [-1, 1], [-1],
                [-1, 1, args.max_seq_len], [-1, 1]],
            dtypes=['int64', 'int64', 'int64', 'float32', 'int64', 'int64', 'float32',
                'int32', 'float32', 'int64'],
            lod_levels=[0, 0, 0, 0, 2, 2, 2, 0, 0 ,0],
            name=pyreader_name,
            use_double_buffer=True)

        (src_ids, sent_ids, pos_ids, input_mask, tgt_ids, tgt_pos, init_scores, parent_idx,
            tgt_input_mask, data_ids) = fluid.layers.read_file(pyreader)
        role_ids = None
        turn_ids = None
    ...
    with while_op.block():
        ...
        dec_out = ernie.encode(pre_ids, pre_pos, type_ids, pre_mask, parent_idx, store=True, role_ids=role_type_ids, turn_ids=turn_type_ids)
        fc_out = cal_logit(dec_out, None, args, ernie_config)
        probs = layers.softmax(fc_out / args.temperature)
